...
from states.high_state import HighPriceState
...
def take_city(message: types.Message):
    ...
    bot.send_message(message.from_user.id, "Вы решили выбрать самые дорогие отели.\n"
...
def choose_city(message):
    ...
    if city_name.replace(" ", "").replace("-", "").isalpha():
        ...
        bot.set_state(message.from_user.id, HighPriceState.check_city, message.chat.id)
...
@bot.message_handler(state=HighPriceState.check_city)
def city_check(message):
    ...
    bot.set_state(message.from_user.id, HighPriceState.check_in, message.chat.id)


@bot.callback_query_handler(
    func=lambda call: call.data.startswith(calendar_1_callback.prefix),
    state=HighPriceState.check_in
)
    ...
    if action == "DAY":
        ...
        with bot.retrieve_data(call.message.chat.id, call.message.chat.id) as hotels_data:
            if datetime.date(int(year), int(month), int(day)) >= datetime.date.today():
                ...
                bot.set_state(call.message.chat.id, HighPriceState.check_out)
            else:
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith(calendar_1_callback.prefix),
    state=HighPriceState.check_out
)
    ...
    if action == "DAY":
        ...
        with bot.retrieve_data(call.message.chat.id, call.message.chat.id) as hotels_data:
            if datetime.date(int(year), int(month), int(day)) >= hotels_data['check_in']:
                ...
                bot.set_state(call.message.chat.id, HighPriceState.count_hotel)
            else:
...
@bot.callback_query_handler(func=None, state=HighPriceState.count_hotel)
def find_all_deals(call):
    ...
    bot.set_state(call.message.chat.id, HighPriceState.info)
    show_info(call)
